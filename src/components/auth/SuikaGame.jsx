...
export default function SuikaGame() {
    ...
    useEffect(() => {
        const loadHighScore = async () => {
            ...
            if (user) {
                ...
                if (userDoc.exists() && data && data.suikaHighScore_3) {
                    setHighScore(data.suikaHighScore_s3);
                } else {
                    await setDoc(userDocRef, { suikaHighScore_s3: 0 }, { merge: true });
                }
            }
        };
        loadHighScore();
    ...
    const updateHighScore = async (newScore) => {
        ...
        if (user && newScore > highScore) {
            ...
            const userDocRef = doc(db, 'users', user.uid);
            try {
                await updateDoc(userDocRef, { suikaHighScore_s3: newScore });
                ...
                if (error.code === 'not-found') {
                    try {
                        await setDoc(userDocRef, { suikaHighScore_s3: newScore });
                        setHighScore(newScore);
